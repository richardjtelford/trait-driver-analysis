...
```{r trait-cor, fig.height=7, fig.cap="Correlation coefficient between each trait combination. Correlation coefficients are shown for trait combinations with a significnat correlarion."}
readd(trait_corr)
```
...
```{r hist-all-1, eval = FALSE, fig.height=7, fig.cap="Histogram for mean trait values over time."}
...
```{r hist-all-2, eval = FALSE, fig.height=7, fig.cap="Histogram for mean trait values over time."}
...
```{r hist-all-3, eval = FALSE, fig.height=7, fig.cap="Histogram for mean trait values over time."}
...
df_f %>% select(-plasticity, -direction) %>% 
...
```{r full-happymoment-plot, fig.cap="Change in mean, variance, skewness, kurtosis and range for leaf area, thickness, phosphorus, and C and N isotopes in each treatment and over time. Shown are mean values for each time period and climate treatment with standard error."}
...
```{r nike-1, fig.cap="Skewness versus kurtosis for all traits in the control plots across the gradient."}
...
```{r nike-2, fig.cap="Skewness versus kurtosis for all traits and treatemnts across the gradient."}
...
moment_summary %>% 
  kbl(booktabs = TRUE,
      longtable = TRUE,
      caption = "Mean and SE of each moment, trait and destination site combination in 2016.") %>%
  kable_styling(latex_options = c("repeat_header"), font_size = 7)
