...
moments_by_climate_table %>%
  kableExtra::kable(caption = "Results from regression of traits and summer air temperature. Shown are estimates (slope), standard error, t value and P value. Significant P values are indicated with asterisks: ** P value < 0.01, * p < 0.05")
...
```{r rda_plot, eval = FALSE, echo = FALSE, fig.cap="Testing for convergence in multivariate community (first column) or trait-space (second and third column) as a function of year since transplant. Results from  Principal Response Curve multivariate RDA analysis. "}
readd(TraitRDA)
```
...
```{r hist-all-1, eval = FALSE, echo = FALSE, fig.height=7, fig.cap="Histogram for mean trait values over time."}
...
treatment_effect_table %>%
  ...
  #booktab = TRUE, longtable = TRUE,
   #kableExtra::kable(caption = "Results Testing for convergence and divergence in each pair-wise trait value as a function of year since transplant. Significant P values are indicated with asterisks: ** P value < 0.01, * p < 0.05") #%>% 
